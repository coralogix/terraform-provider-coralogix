...
**Fix:** When order has no meaning and the attribute is an already-released `ListNestedAttribute`, reorder the API response to follow the prior list before flatten. Use the plan in Create/Update and the state in Read. Match exact content first, then the key alone for drift, because keys can repeat (the API accepts two targets with the same connector and preset). Match each prior item once, and put unmatched items last in API order. See `orderTargetsLike` in `internal/provider/notifications/resource_coralogix_global_router.go`.
