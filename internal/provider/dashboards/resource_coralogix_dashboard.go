...
func flattenDashboardVariableDefinition(ctx context.Context, variableDefinition *cxsdk.DashboardVariableDefinition) (*DashboardVariableDefinitionModel, diag.Diagnostics) {
	...
	case *cxsdk.DashboardVariableDefinitionConstant:
		// constant_value is deprecated and rejected on write; present a legacy or
		// imported Constant as the supported multi_select+constant_list shape so it
		// can be reconciled in config instead of round-tripping to the dead attribute.
