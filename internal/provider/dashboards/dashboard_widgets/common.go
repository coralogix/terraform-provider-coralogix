...
// The IBM and provider-agnostic arms name their field `regionField` on the
// wire, unlike the AWS arm's `awsRegionField`, so the attribute names differ
// too rather than being made uniform.
