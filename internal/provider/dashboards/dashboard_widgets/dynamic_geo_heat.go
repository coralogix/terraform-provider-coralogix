			...
			"config": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					"all_region_config": schema.SingleNestedAttribute{
						...
						MarkdownDescription: "Takes the geographic information from a field holding region values of any supported cloud provider. This is a preview alternative.",
					},
					...
					"ibm_region_config": schema.SingleNestedAttribute{
						...
						MarkdownDescription: "Takes the geographic information from a field holding IBM Cloud region values. This is a preview alternative.",
					},
				},
				Validators: []validator.Object{
...
// The selected arm stays present even when its only field is absent: the API
// preserves `{"ibmRegionConfig":{}}`, so dropping the arm here would make the
// config read back as no arm at all and diff forever.
