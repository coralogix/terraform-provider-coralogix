...
func TestAccCoralogixResourceDashboardVariablesV2LogsFieldName(t *testing.T) {
	name := dashboardOpenAPIFixtureName(t.Name())
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
  ...
  variables_v2 = [{
    name         = "log_fields"
    display_name = "Log fields"
    source = {
      query = {
        all_option = { include_all = false }
        logs_query = {
          type = { field_name = { log_regex = ".*" } }
        }
      }
    }
    value = { multi_string = { selected_all = {} } }
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(dashboardResourceName, "variables_v2.0.source.query.logs_query.type.field_name.log_regex", ".*"),
				),
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PostApplyPostRefresh: []plancheck.PlanCheck{plancheck.ExpectEmptyPlan()},
				},
...
func TestAccCoralogixResourceDashboardVariablesV2SpansFieldName(t *testing.T) {
	name := dashboardOpenAPIFixtureName(t.Name())
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
  ...
  variables_v2 = [{
    name         = "span_fields"
    display_name = "Span fields"
    source = {
      query = {
        all_option = { include_all = false }
        spans_query = {
          type = { field_name = { span_regex = ".*" } }
        }
      }
    }
    value = { multi_string = { selected_all = {} } }
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(dashboardResourceName, "variables_v2.0.source.query.spans_query.type.field_name.span_regex", ".*"),
				),
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PostApplyPostRefresh: []plancheck.PlanCheck{plancheck.ExpectEmptyPlan()},
				},
