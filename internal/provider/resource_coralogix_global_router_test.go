		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckTypeSetElemNestedAttrs(globalRouterResourceName, "rules.*.targets.*", map[string]string{
						"connector_id": fmt.Sprintf("http-%v", name),
						"preset_id":    fmt.Sprintf("http-%v", name),
					}),
					resource.TestCheckTypeSetElemNestedAttrs(globalRouterResourceName, "rules.*.targets.*", map[string]string{
						"connector_id": fmt.Sprintf("slack-%v", name),
						"preset_id":    fmt.Sprintf("slack-%v", name),
					}),
					resource.TestCheckTypeSetElemNestedAttrs(globalRouterResourceName, "rules.*.targets.*", map[string]string{
						"connector_id": fmt.Sprintf("pd-%v", name),
						"preset_id":    fmt.Sprintf("pd-%v", name),
					}),
				),
			},
			{
