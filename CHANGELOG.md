...
- FIX: `decimal_precision` on a `dynamic` widget visualization accepts any value the API's 32-bit field can hold, instead of only 0 to 15. The documented range is not enforced by the API: 0, 15, 16, 400, -1 and both int32 extremes are all accepted and read back unchanged against a live environment, and the API stores -1. The classic widgets' `decimal` reached that conclusion in the previous release, so `20` applied on a line chart and failed on a `dynamic` widget, with nothing explaining the difference. The int32 bound stays, because the conversion to the API's field is an unchecked cast and a larger value would wrap silently between plan and apply.
- FEAT: A `dynamic` widget spans query filter takes an `observation_field`, and its `field` is no longer required. The API models the two as independent fields of `SpansFilter`, with no `oneof` and no required entry, and the Coralogix UI writes a filter that sets only the observation field, so such a dashboard could not be expressed: the plan failed with `attribute "field" is required`. The dynamic widget gets its own filter list for this, because the shared spans filter schema also decodes state written by schema v1 to v3, whose shape must stay frozen. The same gap remains on the classic widgets and on a dashboard-level spans filter for the same freezing reason.
...
- FIX: The same applies to every other union the widget exposes: a geomap aggregation, colour or coordinate source, a bar chart's `sort_order.strategy` and a stat card's `color_label_mapping` stored with no alternative selected now read back unset rather than as a block no configuration can express.
- FIX: A stat card's `color_label_mapping.range.min_max` now follows the same rules as everywhere else: `auto = false` is rejected rather than silently selecting the custom range.
...
- FIX: A marker attribute such as `aggregation.count`, `min_max.auto`, `sort_order.strategy.category` or `mapped_values` set to `false` is now rejected rather than treated as unset. Set to false it selects nothing while staying present in the configuration, which the API cannot represent, so the apply failed with an inconsistent-result error. Omit the attribute instead.
- DOC: `gauge.arc_display` and `pie_chart.legend` sit alongside the older `show_inner_arc`, `show_outer_arc` and `show_legend` attributes. The API stores both independently and the Coralogix UI honours the newer attribute, verified against a live environment, so no conflict validator is needed and the older attributes keep working untouched.
- FIX: The enum attributes this release adds — `x_axis_time_format`, `bar_value_display`, `legend_by`, `editor_mode`, `series_limit_type`, `promql_query_type` and the metrics `aggregation` — are `Optional + Computed` with no static default. The API picks a value when the attribute is omitted, so a default of `unspecified` claimed a value the API does not use and made the first plan after upgrading show a change on dashboards created by an earlier release. Set `unspecified` explicitly to hand the choice back to the API. An enum the API does not set now reads back as `unspecified` rather than an empty string, which the schema does not allow and which left the attribute planning as unknown for ever. `legend.placement` had the same shape and is fixed with them, so a `legend` that omits `placement` no longer leaves a permanent diff.
...
- FEAT: Add the remaining `gauge` display and query fields: `arc_display`, `custom_unit`, `decimal_precision`, `legend`, `legend_by` and `show_min_max` on the widget; `group_by` on the logs query and `group_by` plus `group_bys` on the spans query, for grouping the gauge value; and `editor_mode` and `promql_query_type` on the metrics query.
- FEAT: Add the remaining `pie_chart` display and query fields: `custom_unit`, `decimal`, `decimal_precision`, `legend` and `show_total` on the widget; `aggregation`, `editor_mode` and `promql_query_type` on the metrics query; and `group_names_fields` and `stacked_group_name_field` on the spans query.
- FEAT: Add the remaining `bar_chart` and `horizontal_bar_chart` display and query fields. Both gain `custom_unit`, `decimal`, `decimal_precision`, `legend`, `y_axis_max` and `y_axis_min`; the bar chart also gains `bar_value_display` and `x_axis_time_format`. Their shared metrics query gains `aggregation`, `editor_mode` and `promql_query_type`, and their shared spans query gains `group_names_fields` and `stacked_group_name_field` for grouping by a span observation field with an explicit scope.
- FEAT: Add the remaining `line_chart` display and query fields: `connect_nulls`, `use_data_time_range` and `x_axis_time_format` on the widget; `custom_unit`, `decimal`, `decimal_precision`, `y_axis_max` and `y_axis_min` on each query definition; `group_bys` on the logs and spans queries, for grouping by an observation field with an explicit scope, alongside the existing `group_by`; and `editor_mode` and `series_limit_type` on the metrics query. A dashboard that set any of them in the Coralogix UI can now be expressed in Terraform.
...
- FIX: The gauge's deprecated `show_inner_arc` and `show_outer_arc` now warn at plan time instead of only in the documentation.
...
- FEAT: Add the `vertical_bars`, `vertical_bars_multi`, `horizontal_bars` and `horizontal_bars_multi` visualizations to the `dynamic` widget, including per-query `query_field_settings` and the `sort_order` strategy.
- FIX: `sort_order.strategy.category` is a `true`/absent switch rather than a JSON object. The API models it as an empty message, so a JSON value was rejected outright and the strategy could not be selected.
- FIX: Every list the bar visualizations expose rejects an explicit empty value at plan time instead of failing the apply with an inconsistent-result error.
