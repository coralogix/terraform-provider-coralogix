...
- FIX: Reject a null `connector_config.field_values_wo_versions` entry. It recorded nothing, so a later change to the secret alone was never sent. An empty `field_values_wo` no longer requires a versions map either, which a module passing a variable hits on its default.
