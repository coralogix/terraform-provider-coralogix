...
- FIX: Omitting `labels` now clears them on update. The attribute was `Optional+Computed` with `UseStateForUnknown()`, so removing it from HCL copied the prior map into the plan and Replace kept the old labels.
