...
- FIX: The `dynamic` widget now enforces the lengths and list sizes the API documents: `custom_unit` is 1 to 128 characters, a tooltip `message_template` is 1 to 4096, and the lists the dynamic widget declares itself are capped at 1000 items. Lists it shares with the legacy widgets - an observation field's `keypath`, a legend's `columns`, query `filters` and a filter's `selected_values` - are left uncapped for now, since those resources have shipped for years and the API does not enforce the limit. The API accepts larger values and stores them, so these reject a configuration that previously applied — most likely to be noticed on a `custom_unit` longer than 128 characters, which has been settable on the `stat` visualization since 3.10.0. An explicitly empty `custom_unit` or `message_template` is also now rejected, matching the documented minimum of one character; the API stores an empty value today, so remove the attribute instead of setting it to an empty string.
