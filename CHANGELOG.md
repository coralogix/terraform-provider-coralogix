...
- FEAT: Add typed HCL support for the `dynamic` widget definition, including logs/spans/metrics/data_prime query definitions, a top-level `time_frame`, and the `stat`, `stat_card`, `table`, `time_series_lines_multi`, `time_series_bars`, `vertical_bars`, `vertical_bars_multi`, `horizontal_bars`, `horizontal_bars_multi`, `gauge`, `pie_chart`, `hexagon_bins`, `heatmap`, `geomap` and (deprecated) `time_series_lines` visualizations (full field fidelity). The deprecated top-level singular `query` field is intentionally not modelled; use `query_definitions`, which carries the same queries and is what the API stores.
- FIX: Spans query filters now round-trip their `observation_field`, and the `simple_value` legend column is no longer dropped on read — both were previously silently lost on import for every widget that uses them. Dashboards that use a `simple_value` legend column will therefore show a one-time corrective diff on the first refresh after upgrading, replacing the empty string previously written to state with the real value.
- FIX: `dynamic.visualization.heatmap.preset` and `.color_range` are a single API union, but both defaulted to `unspecified`, so every heatmap send populated both arms and create/update failed with `at most one of [preset, colorRange] may be set`. Both are now plain optional and mutually exclusive, making the visualization usable.
- FIX: `dynamic.visualization.table.rules[*].properties[*].definition` is an API union, but `alignment` defaulted to `unspecified` and so was always populated, making every other arm (`column_display_name`, `regex_extract`, `link`, `thresholds`, `units`, `values_alias`, `values_mapping`) fail with `at most one of [...] may be set`. `alignment` no longer defaults and the union now requires exactly one arm at plan time.
- FIX: `dynamic.visualization.table.rules[*].rule_scope` has the same problem via `field_type`; it no longer defaults and the union now requires exactly one of `field`, `regex` or `field_type`.
