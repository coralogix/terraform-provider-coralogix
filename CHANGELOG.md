...
- FEAT: Add the remaining `line_chart` display and query fields: `connect_nulls`, `use_data_time_range` and `x_axis_time_format` on the widget; `custom_unit`, `decimal`, `decimal_precision`, `y_axis_max` and `y_axis_min` on each query definition; `group_bys` on the logs and spans queries, for grouping by an observation field with an explicit scope; and `editor_mode` and `series_limit_type` on the metrics query. A dashboard that set any of them in the Coralogix UI can now be expressed in Terraform.
