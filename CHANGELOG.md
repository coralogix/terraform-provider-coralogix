...
- FIX: Accept an empty `custom.headers_wo` without requiring `custom.headers_wo_versions`, which a module passing a variable hits on its default. Mismatched keys are still rejected. A plan no longer reports a missing version when only one of the two maps is still unknown; the comparison waits until both resolve.
