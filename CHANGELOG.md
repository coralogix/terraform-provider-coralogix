...
- FIX: A value that is unknown while planning and resolves to something invalid during apply - an enum outside its allowed set, a number outside its range, a list that resolves to empty - is now rejected with a message naming the attribute, instead of being silently dropped or failing the apply with an inconsistent-result error.
