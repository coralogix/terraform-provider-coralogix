...
- FIX: `data.coralogix_dashboard` no longer fails to return a dashboard whose widgets use an attribute with a custom value type. The resource-to-data-source schema conversion dropped `CustomType` for numeric attributes, so the data source declared a plain type while reads produced the custom one. Affects any resource whose data source derives its schema this way.
