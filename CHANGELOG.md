...
- FIX: A `visualization` block that resolves to two visualizations after apply, or to none, is now reported instead of silently keeping one or omitting the widget's visualization entirely. The same applies to a `min_max` that resolves to both of its alternatives.
