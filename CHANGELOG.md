...
- FEAT: Add typed HCL support for the `dynamic` widget definition, replacing `content_json` as the only way to manage these widgets. Covers `query_definitions` with the logs/spans/metrics/data_prime query union, a top-level `time_frame` and `interpretation`, and the `stat` and `stat_card` visualizations at full fidelity. Previously every `dynamic` widget failed import and data-source reads. Visualizations not yet supported, and the deprecated top-level `query`, fail the read with a clear diagnostic rather than writing state that silently drops them. Configurations the API rejects are now caught at plan time with the attribute named, rather than at apply: a colour mapping needs exactly one of `range`, `value` and `regex`; an element cannot both read a field and show the mapping result; `primary_value` cannot show the mapping result at all; and every list rejects an explicit empty value.
- FIX: `unit` now accepts `percent` and `datetime_iso`, which the API supports but the shared unit map omitted. A widget saved with either — for example by the Coralogix UI — read back as null and the next apply overwrote it with `unspecified`. The gauge unit map was also missing `datetime_iso`. Affects every widget type with a `unit`, not only the dynamic one.
