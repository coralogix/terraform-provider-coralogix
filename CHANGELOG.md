...
- FIX: Deprecate the dashboard variable `constant_value` attribute. It maps to the API's deprecated `Constant` variant, which the backend rejects with an opaque `invalid variable definition: Constant(...)` error. A `DeprecationMessage` now surfaces this at plan time and directs users to a `multi_select` variable with a `constant_list` source and a single `selected_values` entry — the supported replacement.
