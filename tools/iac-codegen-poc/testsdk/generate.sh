#!/bin/bash
# Generates the test SDK from spec/fake/*.yaml with the same steps,
...
# Only testsdk/go/openapi/cxsdk is handwritten.
#
# Run from the repository root: testsdk/generate.sh
set -euo pipefail
...
generate_package() {
  local spec="$1" pkg="$2"
  local out="testsdk/go/openapi/gen/$pkg"
  local work
  work="$(mktemp -d)"
  mkdir -p "$work/out"
