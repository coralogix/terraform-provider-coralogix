...
Unchanged Terraform attributes are not a compatibility proof. Do these checks before the PR:
...
3. **API-key permissions.** List every RPC the new create/update/read path calls. Map each to a permission in `mapRoleToPermission` (`internal/provider/aaa/resource_coralogix_api_key.go`). A new lookup on another service (name → id) needs that service's permission. Existing least-privilege keys that could do the old write will fail before the intended API. Prefer sending the name on the original service over a lookup on another service. If a lookup is required, document the extra permission as a breaking change. Acc tests with a full admin key do not catch this.
...
- **What a read returns decides the schema shape:** Before choosing `Optional`, find out what the API returns for a resource created outside Terraform, from a real read and not from an assumption. If it never returns a value, plain `Optional` is right. If it always returns one, the attribute has to be `Optional+Computed`, or the apply fails with "was null, but now ...". If it returns one only sometimes, the plan modifier decides the rest: `UseNonNullStateForUnknown()` when a null in state must become unknown so the API can fill it in, and `UseStateForUnknown()` when a null in state means the value is truly absent and has to stay absent. A static default is never the answer for a proto wrapper field, because absent is not zero and the default sends a value the resource never had. Losing the ability to clear the value is the price of `Computed`: document how to change it, because deleting the line no longer clears it.
- **Plan/state round-trip:** For every schema field touched, trace schema → model → extract/expand → API → flatten → state. Watch for defaults that the API ignores, backend-generated IDs that need `Optional+Computed`, proto `UNSPECIFIED` enum values leaking into state, null vs empty list drift, and schema versions that were not updated together. If an API union treats an omitted or empty list as `all`, default and flatten it to the same known empty list; reject empty lists for branches that require values.
- **Prior schema versions freeze every shape they reference:** A resource with `UpgradeState` keeps its old schemas so stored state can still be decoded. A helper that builds attributes is often shared between the current schema and those old ones, so adding or removing an attribute through that helper changes the type of state that is already on disk. Adding a new attribute to a shape that only the current version needs means giving the current version its own schema, model, attribute-type map, expand and flatten, and leaving the shared one alone. Making an attribute optional instead of required, or adding a validator, is safe: neither changes the type. When the current version's type stops matching a prior version's, every state upgrader that carried that value over unchanged has to rebuild it instead, with the new attribute set to null.
...
- **CRUD error paths:** After `resp.Diagnostics.AddError`, return immediately. A failed create/update must not continue into flatten/state writes, because empty IDs or zero-value state can poison later reads and plans. `Delete` must treat HTTP 404 / `codes.NotFound` as success (return with no diagnostic; Framework then drops state). Nil-guard OpenAPI `httpResponse`. Read/Update still warn and `RemoveResource`.
