...
- **CRUD error paths:** After `resp.Diagnostics.AddError`, return immediately. A failed create/update must not continue into flatten/state writes, because empty IDs or zero-value state can poison later reads and plans. A resource `Delete` must treat HTTP 404 / `codes.NotFound` as success (return with no diagnostic; Framework then drops state). Resource Read/Update can warn and `RemoveResource`. A data source must return an error when its object is not found; removing data-source state produces an invalid null result and Terraform reports `Provider produced null object`. Nil-guard OpenAPI `httpResponse`.
