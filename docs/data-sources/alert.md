...
- `data_sources` (Attributes List) Data sources to associate the alert with. Only supported for log-based alert types. The referenced data space and dataset must already exist. The API currently supports at most one entry. (see [below for nested schema](#nestedatt--data_sources))
- `deleted` (Boolean)
