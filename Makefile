...
GROUP_MIGRATION_ACC_PATTERN=^TestAccCoralogixResourceGroupMigration
ACC_SKIP_PATTERN?=${DASHBOARD_ACC_PATTERN}|${EVENTS2METRIC_MIGRATION_ACC_PATTERN}|${GROUP_MIGRATION_ACC_PATTERN}
...
testacc-group-migration:
	CORALOGIX_GROUP_MIGRATION_ACC=1 TF_ACC=1 TF_ACC_PROVIDER_NAMESPACE=coralogix go test ${BUILD_ARGS} ./internal/provider -run '${GROUP_MIGRATION_ACC_PATTERN}' -v -count=1 -timeout 120m -parallel=3

testacc-provider-migration:
	CORALOGIX_DASHBOARD_MIGRATION_ACC=1 CORALOGIX_EVENTS2METRIC_MIGRATION_ACC=1 CORALOGIX_GROUP_MIGRATION_ACC=1 TF_ACC=1 TF_ACC_PROVIDER_NAMESPACE=coralogix go test ${BUILD_ARGS} ./internal/provider -run '${DASHBOARD_MIGRATION_ACC_PATTERN}|${EVENTS2METRIC_MIGRATION_ACC_PATTERN}|${GROUP_MIGRATION_ACC_PATTERN}' -v -count=1 -timeout 120m -parallel=3
